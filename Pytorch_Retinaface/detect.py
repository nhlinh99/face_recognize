...
def inference(model_detection, input_image, threshold_detect, cfg, device):
    
    image = input_image.astype(np.float32).copy()
    top_k = 100
    ...
    # ignore low scores
    inds = np.where(scores > threshold_detect)[0]
    boxes = boxes[inds]
    landms = landms[inds]
    scores = scores[inds]
    ...
    crop_faces = []
    for det in dets:
        landmarks = [[det[k], det[k+1]] for k in range(5, 15, 2)]
        crop_face_img = process(input_image, landmarks, (112,112))
        crop_faces.append(crop_face_img)

    return dets, crop_faces
...
if __name__ == '__main__':
    ...
    # testing begin
    for i in range(1):
        image_path = "C:/Users/linhnh/Desktop/TMT/dataset/face/THDQuang/sample.mp4_Color_1653455242870.53393554687500.png"
